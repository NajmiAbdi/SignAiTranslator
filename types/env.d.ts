  ...
  <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-100">
    ...
    <div className="h-48 flex items-end justify-between space-x-1">
      {data.slice(-7).map((item, index) => {
        const maxValue = Math.max(...data.map(d => d.value || d.translations || d.users || 1));
        const height = ((item.value || item.translations || item.users || 0) / maxValue) * 160;
        
        return (
          <div key={index} className="flex flex-col items-center flex-1">
            <div 
              className={`${type === 'bar' ? 'bg-secondary-600' : 'bg-primary-600'} rounded-t w-full min-h-[4px]`}
